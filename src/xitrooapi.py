...
class xitroo():
    def __init__(self, email, refresh_counter=5, refresh_wait=5):
        self.email = email
        ts = int(format(time.time(), ".0f"))
        for i in range(refresh_counter):
            idsite = requests.get(f"https://api.xitroo.com/v1/mails?locale=de&mailAddress={self.email}&mailsPerPage=25&minTimestamp=0.0&maxTimestamp={str(ts + 500)}").json()
            try:
